...
> 🧠 Your thinking partner for multiple complex projects  
> 🎯 把 OKR 变成任务、可视化时间线、风险雷达与每日 insight 的项目助手

| 🎯 Goal To Plan | 📅 Planning Views | ⚠️ Risk Radar | 💡 Daily Insight |
| --- | --- | --- | --- |
| 把 OKR 拆成可执行任务、依赖和排期 | 生成甘特图、依赖图与任务表 | 根据工作总结自动识别滞后与风险 | 持续补充来自论文、开源和社区的外部输入 |
...
6. 生成计划后查看甘特图、依赖图和任务表。
7. 指定团队工作总结文件夹，增量更新任务状态。
8. 打开右侧 Assistant，对任务规划或完成状态进行问答、修改或重规划。
9. 点击右下角“生成insight”，等待两分钟左右即可查看基于这个任务查询到的相关外部资料。点击卡片标题即可进入来源链接。
...
| 项目推进时只盯内部执行，缺少来自最新论文、开源项目、博客社区的外部输入 | 按天推荐高相关 insight card |
...
- 外部社区输入走的是 `reddit_read_only` 只读链路（也可在设置中手动关闭reddit功能），只获取公开内容，不提供发帖、回复、投票或管理等写操作
...
- `💡` 结合项目上下文，为你生成今日 1-3 张 insight card
...
- `完成周数`：例如输入 `12`，生成/重生成时会把计划总时长锁定为 12 周左右。
- 该约束会同时传给模型（提高初稿命中率）和排期器（保证最终总时长对齐）。
- 该约束仅在生成/重生成时生效，不会在日常手工微调时强制重压缩。
> [!NOTE]
> 当前的prompt设计可能会让输出计划的总时长与预设产生轻微差别。如果您需要完全固定的总计划时长，可以考虑与assistant对话做微调，或在/planner/agents目录下修改plan_builder和review_refine_agent的提示词（但这可能带来模型推理的大量增长，并增加一次任务生成所需时间）。
...
你可以点击右下角“生成今日 Insight”按钮，基于当前任务上下文生成 1-3 张高相关 insight card。来源可配置，默认覆盖：
...
- 询问任务、依赖、总结、风险
- 修改任务名、负责人、状态、进度、工期、排期周区间（如 W7-W8->W9-W11）
- 新增任务 / 删除任务
- 修改依赖关系
- 一句话提出多个修改请求
- 对当前计划提出整体重规划请求
...
- `Progress 并发数` 最好和所选 Provider 的限速能力匹配。
...
- 外部输入筛选时效、assistant上下文长度等为默认值，如有需要，可以在代码中更改。
- task/insight正在生成时，建议先去干点别的事情，不要对页面做额外操作，以免引发不必要的前端问题。