...
### 把 OKR 变成任务、时间线、风险雷达与每日 insight 的项目助手

Project Copilot 是一个基于 Streamlit + LiteLLM 的本地优先项目助手，围绕下面这条主线工作：

`规划 -> 跟进 -> 风险识别 -> 外部输入 -> 对话式调整`

它不是一个只会“吐一份任务列表”的 demo，而是把 OKR 拆解、自动排期、工作总结解析、风险预警、insight 推送、记忆管理和聊天式修改放进同一个项目上下文里，方便你持续管理复杂项目。
...
2. 新建一个项目。
3. 粘贴你的 OKR。
4. 选择 `单Agent` 或 `多Agent` 任务生成模式。
5. 生成计划后查看时间线、依赖图和任务表。
6. 指定团队工作总结文件夹，增量更新任务状态。
7. 打开右侧 Assistant，对任务规划或完成状态进行问答、修改或重规划。
...
- OKR 写得很好，但落不到可执行任务和合理排期上
- 周报、日报、会议纪要很多，没人愿意手动逐项更新任务进度
- 风险往往不是没有数据，而是项目负责人没时间把分散的信息拼起来
- 项目推进时只盯内部执行，缺少来自论文、开源项目、博客社区的外部输入
- 想用自然语言修改任务，但又不希望 assistant 乱改状态、直接落盘

这个项目对应给出了一套相对克制、可控的方案：
...
- 结合项目上下文，每天生成 1-3 张 insight card
...
项目会根据当前任务上下文，每天生成 1-3 张高相关 insight card。来源可配置，默认覆盖：
...
![alt text](<result (1).jpeg>)
![alt text](<result (2).jpeg>)
...
![alt text](<okr_project_dependency_graph (2).png>)
...
Project Copilot 会结合当前项目内容推送 1-3 张 insight card，并允许你长期积累偏好和批注。
![alt text](<result (3).jpeg>)
...
- `config.json` 负责保存 Provider、Base URL、模型和并发等非敏感配置
...
```json
{
	...
	"providers": {
		"zhipu": {
			...
			"model": "glm-4.6",
			"assistant_model": "glm-4.5-airX",
			"progress_model": "glm-4.5-airX",
			"max_tokens": 7000,
			"progress_max_workers": 5,
...
- `Progress 并发数` 不要一味调高，最好和你所选 Provider 的限速能力匹配。
- 同一个项目尽量长期复用同一个“工作总结文件夹”，这样增量更新最稳定。
- 风险阈值可以先从 `30%` 开始，再根据团队节奏调整灵敏度。
- 如果你发出的修改类指令没有真正生效，先看是不是还停留在“待执行”阶段；需要回复 `确认执行` 才会落盘。
